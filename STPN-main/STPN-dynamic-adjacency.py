...
class DynamicAdjacency(nn.Module):
    def __init__(self, node_num, hidden_dim):
        ...
        """
        # 关系强度预测层（可选）
        self.proj = nn.Linear(2 * hidden_dim, 1) if hidden_dim > 1 else None
        """
        如果hidden_dim大于1，还定义了一个线性层proj，输入是2*hidden_dim，输出是1。
        这一步可能用于后续计算节点间的关系强度，但暂时在forward中没有看到使用这个proj层，可能后续有其他用途或者备用。
        """
    def forward(self):
        # 归一化嵌入向量 [V, D]
...
# 然后结合机场之间的关系，来做更聪明的时空预测。”
    ...
    Shape:
        ...
        where
            ...
            +
    """
    def __init__(self, in_channels, out_channels, emb_size, dropout, time_d = 16, heads = 4, support_len = 3, order = 2, final_layer = False, dynamic_dim=64, node_num=30):
        """
        ...
        self.fusion_weights = nn.Parameter(torch.ones(support_len + 1) / 2)  # 原始支持矩阵数+动态矩阵
        """
...
class STPN(nn.Module):
    ...
    def __init__(self, h_layers, in_channels, hidden_channels, out_channels, emb_size, dropout, wemb_size = 4, time_d = 4, heads = 4, support_len = 3, order = 2,
                 num_weather = 8, use_se = True, use_cov = True, dynamic_dim=64, node_num=30):
