...
# Step 1: 创建两个简单的四维张量
tensor1 = torch.randn(2, 3, 4, 1)  # 形状 (2, 3, 4, 5)
tensor2 = torch.randn(2, 3, 4, 1)  # 形状 (2, 3, 4, 5)
print(tensor1)
print(tensor2)
# Step 2: 选择第一个机场的数据
tensor1_last = tensor1[:, 0, :, -1]  # 选择第一个机场的数据，形状 (2, 4)
tensor2_last = tensor2[:, 0, :, -1]  # 选择第一个机场的数据，形状 (2, 4)
print(tensor1_last)
print(tensor2_last)
# Step 3: 将张量展平为二维数组用于绘图
tensor1_last_flat = tensor1_last.reshape(-1)  # 展平
tensor2_last_flat = tensor2_last.reshape(-1)  # 展平
print(tensor1_last)
print(tensor2_last)
# Step 4: 绘制曲线图
...
# 绘制 tensor1 的真实值
plt.plot(tensor1_last_flat.numpy(), label='True Values (Tensor 1)', color='blue', linestyle='-', marker='o')

# 绘制 tensor2 的预测值
plt.plot(tensor2_last_flat.numpy(), label='Predicted Values (Tensor 2)', color='red', linestyle='--', marker='x')

# 添加标题和标签
plt.title('Real vs Predicted Values for the First Airport')
...
# 然后结合机场之间的关系，来做更聪明的时空预测。”
    ...
    def forward(self, x, t_in, supports, t_out = None):
        """
        x：输入数据 [N, C, V, T_in]、t_in：输入时间标签 [N, T_in]
        supports：邻接矩阵（支持多个）
...
class SELayer(nn.Module):
    """通过全局池化与门控机制，动态调整特征通道权重（图6），例如在恶劣天气时增强天气特征的影响。
...
class STPN(nn.Module):
    ...
    def forward(self, x, t_in, supports, t_out, w_type):
        if self.use_cov:
            # 新代码
            w_vec = self.w_fc(w_type)  # 输入shape: (batch, N, T, 14)
